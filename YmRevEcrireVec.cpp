                    ...
                    case 2:
						...
						/*
							...
							if (VerificationCodeEquipementResaven(codeEquipResaven) == 0){
								...
								else cout<<"pSegment pas initialisé : "<<_pSegments<<endl;
								cout<<"done"<<endl;
								break;
							}
						...
						*/
                            ...
                            if (struc_prec == 4)  {
                                ...
                                //if (_iAuto) this->AnnulationSegmentTemporairePrecedent(); JRO IC SRO DT 23805 : La seule manière de discriminer 
								// un train IC SRO d'une reservation auto train incomplète est de vérifier le code équipement

								//Si le segment précedent ne correspond pas à un TGV ou un CORAIL, c'est à dire à un AUTO TRAIN : //A VERIFIER, pas important 
								//pour le moment car dans tout les cas les trajets non CORAIL ou TGV sont rejeté dans gestionFinTCN
							...
							//Scan du code équipement
							sscanf(&sLigne[112],"%3s",codeEquipResaven);
						//Vérification du typage TGV ou CORAIL , si pas TGV ou CORAIL, on rejete le TCN
                     ...
                     case 4:
                        ...
                        if (!firstTcn) {
                            if (strlen(sLigne) < 95) {
							 ...
							 //JRO DT23805 YIELD IC SRO  Quelle que soit la classe de service, sur 1 ou 2 caractères, le TCN doit être pris en compte et traité
							 ...
							 */
							 ...
							 //JRO IC SRO - Mise en commentaire de la ligne suivante, car dans le cas d'un IC SRO, la struct 4 peut être precedée d'un 02
...
//
                     ...
                     case 10:
                         ...
                         if (!firstTcn) 
						 {
							 ...
							 //JRO IC SRO - DT 23805 - Reg train Auto : Avant IC SRO, tester iAuto était pertinent car 
                             ...
                             //_iAuto=0; JRO IC SRO - DT 23805 - Reg train Auto : _iAuto est désormais toujours à O
...
//MHUE 11/09/06
		...
		//JRO DT23805 IC SRO - Le test suivant n'est plus valable, on prend les trains qu'ils soient yieldé ou non, cad qu'ils aient une classe de controle
		//ou non
...
void YmRevEcrireVec::TraiterSegments(double ldMontantTaxation,int liSegEff, YmRevTypePassager * pTypePassager,int iNumLigne04)
{
    ...
    for (int lis=0;lis<_iSegTemp;lis++) {
        // Début Correction Ano - 98698 : Ignorer tous les segments dont le code équipement n'appartient pas à la liste des code équipement de TGV et Corail
...
void YmRevEcrireVec::GestionFinTcn(int struc_prec,char * sLigne04,int iNumLigne04,int iNumLigne00, long numPalierSegment, Booking StructBooking)
{
    ...
    // Si le numero de structure de la ligne precedente est 04, generation d'instance de PassagerParcours
    if (struc_prec == 4) {
		//Si c'est un train auto, alors c'est incomplet et on supprimer le segment précedent
		// TDR - Anomalie 90705 Segments dont le type d'équipement n'est ni CORAIL, ni TGV
    ...
    // TDR - Anomalie 90705 Si le tcn n'a pas de train TGV ou Corail il n'est pas traité
...
void YmRevEcrireVec::InitialisationNouveauSegmentTemporaire(char *sLigne, long& numpalierRet, Booking *StructBooking)
{
	...
	// Récupérer l'indicateur upghan et la classe de service booking ainsi que le palier booking
